...
void GravitySort::sort(float speed)
{
    ...
    for (auto elem : elems) {
        if (elem > max) {
            this->render(elems, std::distance(elems.begin(), std::find(elems.begin(), elems.end(), elem)), std::distance(elems.begin(), std::find(elems.begin(), elems.end(), elem)), speed);
            if (wantBreak)
                return;
            max = elem;
        }
