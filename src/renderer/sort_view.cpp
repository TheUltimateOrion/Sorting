        ...
        if (auto appShared = m_app.lock())
        {
            ...
            switch (appShared->currentDisplayType)
            {
                case DisplayType::Bar: {
                    SDL_FRect rect {
                        k * t_params.barSpacing, appShared->ctx->winHeight, t_params.barSpacing,
                        -(m_elems[k] * t_params.barSpacing * appShared->ctx->winHeight
                          / appShared->ctx->winWidth)
                    };
                ...
                case DisplayType::Dot: {
                    SDL_RenderPoint(
                        appShared->ctx->renderer, (k + 1) * t_params.barSpacing,
                        (appShared->ctx->winHeight
                         - m_elems[k] * t_params.barSpacing * appShared->ctx->winHeight
                               / appShared->ctx->winWidth)
                    );
                }
                ...
                case DisplayType::RainbowRectangle: {
                    ...
                    SDL_FRect rect {
                        k * t_params.barSpacing, 0, t_params.barSpacing, appShared->ctx->winHeight
                    };
                    SDL_RenderFillRect(appShared->ctx->renderer, &rect);
                }
                break;
                ...
                case DisplayType::Circle: {
                    ...
                    vertices[0] = {
                        {0.5f * appShared->ctx->winWidth,
                         0.5f * appShared->ctx->winHeight}, // first point location
                        {r, g, b, 0xFF},
                        {0.f, 0.f}
                    };
                    ...
                    if (!SDL_RenderGeometry(
                            appShared->ctx->renderer, nullptr, vertices, 3, nullptr, 0
                        ))
                    {
                        LOGERR(SDL_GetError());
                    }
    ...
    void SortView::update(UIState& t_uiState) noexcept
    {
        ...
        if (auto appShared = m_app.lock())
        {
            ...
            RenderParams t_params {
                ...
                .circleRadius    = std::min(appShared->ctx->winHeight, appShared->ctx->winWidth)
                              * 0.75f * 0.5f,
                .spiralScale = 0.5f * std::min(appShared->ctx->winHeight, appShared->ctx->winWidth)
